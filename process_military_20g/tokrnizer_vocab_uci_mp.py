...
def docs_supplier(filename, nums_split=500000):
...
def get_tokenized_dataset_mp(nums_workers, raw_url, token_out_url):
    ...
    :return:
    """
    docs = docs_supplier(raw_url, nums_split=1000)
    log_f = open(token_out_url, "a+")
    t1 = time.time()
    with Pool(nums_workers) as processes:
        # result = processes.starmap(token_docs, zip(docs))
        for result in tqdm(processes.imap(token_docs, docs, chunksize=10), total=5601551/1000):
...
def build_vocab_dataset_mp(num_workers,tokenized_url,vocab_url):
    ...
    with Pool(num_workers) as processes:
        for vocab in tqdm(processes.imap(get_vocab,docs,chunksize=30), total=5601551/1000):
...
def get_UCI_dataset_mp(num_workers,tokenized_url,uci_url,vocab_url):
    ...
    t1 = time.time()
    with Pool(num_workers) as processes:
        for doc_counter_list in tqdm(processes.imap(get_uci_,docs,chunksize=30), total=5601551/10000):
...
def pipeline_preprocess_raw(num_worker):
    ...
    tokenized_data_url = '/home/lcl/LightLDA/military_20g/20g/all_docs_tokenized.log'
    vocab_url = '/home/lcl/LightLDA/military_20g/20g/vocab.military20.txt'
    uci_url = '/home/lcl/LightLDA/military_20g/20g/docword.military20.txt'
    # 得到分词后的数据集
    ...
    # 得到UCI数据格式数据
    get_UCI_dataset_mp(num_worker,tokenized_url=tokenized_data_url,uci_url=uci_url,vocab_url=vocab_url)

if __name__ == '__main__':
    pipeline_preprocess_raw(20)
